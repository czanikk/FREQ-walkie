...
const startSimulator = () => {
  ...
  // Make a few buses offline for demo variety
  [2, 9, 18].forEach(idx => {
...
const moveBus = (bus) => {
  ...
  const trail     = [...(bus.trail || []), [lat, lng]].slice(-150);
  ...
  state.forEach(bus => {
    ...
    if (etaMins <= alertM && etaMins > 0.5) {
      if (!geofenceAlerted.has(alertKey)) {
        ...
        console.log(`Geofence: Bus ${bus.number} is ~${Math.round(etaMins)} min from school`);
      }
    }
  });
};
